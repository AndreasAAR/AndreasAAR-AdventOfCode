...
class ValidPassChecker:
    ...
    def is_correct_helper(self, password,min,max, letter):
        num_min = int(self.clean(min))
        num_max = int(self.clean(max))
        first_is_letter = password[num_min-1] == letter
        second_is_letter = password[num_max-1] == letter
        return bool(first_is_letter) != bool(second_is_letter)
